...
```bash:scripts/fungible-token-v2-2-install.sh
#!/usr/bin/env bash

set -euo pipefail

echo "=========================================================="
echo " FungibleTokenV22 Client SDK Installation & Setup Script"
echo "=========================================================="
...
if [ "$NODE_MAJOR" -lt "$REQUIRED_NODE_MAJOR" ]; then
    echo "[-] Error: Node.js version $NODE_VERSION is too old. Required: >= $REQUIRED_NODE_MAJOR."
    exit 1
fi
echo "[+] Node.js version $NODE_VERSION verified."

# 2. Setup Project Directories
echo "[+] Initializing directory structure..."
mkdir -p src/client
mkdir -p contracts/managed/fungible-token-v2-2/contract
mkdir -p docs
mkdir -p examples
mkdir -p scripts

# 3. Create package.json if not present
...
echo "[+] Installing Midnight runtime & TypeScript dependencies..."
...
npm install \
    ...
    @midnight-ntwrk/midnight-js-contracts@^0.1.0 || true
...
echo "To execute the example: npx tsx examples/fungible-token-v2-2-example.ts"
echo "=========================================================="